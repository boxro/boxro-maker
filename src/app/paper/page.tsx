    ...
    return new Promise((resolve) => {
      const tryCapture = (attempts = 0) => {
        ...
        try {
          ...
          // 고정 크기 설정 (화면 크기와 무관) - 모바일/데스크톱 공통
          const FIXED_WIDTH = 1300;  // 공통 고정 크기
          const FIXED_HEIGHT = 976;  // 공통 고정 크기
  ...
  const createBusType2Shape = () => {
    ...
    const frontViewPage = await createPage('', async (ctx) => {
      ...
      if (carType === 'sedan-type2') {
        ...
        // === 종이접기용 부품들 (간격없이 붙여서 배치) ===
        const totalPartsWidth = frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength + trunkLength + rearLength;
        const partsStartX = centerX - totalPartsWidth / 2;
        
        // 앞면 그리기
        const frontX = partsStartX + frontLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        // 보닛 그리기 (앞면 바로 옆)
        const bonnetX = partsStartX + frontLength + bonnetLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        // 앞창문 그리기 (보닛 바로 옆)
        const frontWindowX = partsStartX + frontLength + bonnetLength + frontWindowLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        // 앞유리 그리기
        const sedanFrontGlassX = frontWindowX;
        ...
        // 지붕 그리기 (앞창문 바로 옆)
        const roofX = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        // 뒷창문 그리기 (지붕 바로 옆)
        const rearWindowX = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        // 뒷유리 그리기
        const sedanRearGlassX = rearWindowX;
        const sedanRearGlassY = topBaseY - carWidth/2;
        ctx.beginPath();
        const sedanRearGlassWidth = rearWindowLength * 0.58;
        const sedanRearGlassHeight = carWidth * 0.78;
        ...
        // 트렁크 그리기 (뒷창문 바로 옆)
        const trunkX = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength + trunkLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(trunkX - trunkLength/2, topBaseY);
        ctx.lineTo(trunkX + trunkLength/2, topBaseY);
        ctx.lineTo(trunkX + trunkLength/2, topBaseY - carWidth);
        ctx.lineTo(trunkX - trunkLength/2, topBaseY - carWidth);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        ctx.font = '16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('트렁크', trunkX, topBaseY + 30);
        
        // 뒷면 그리기 (트렁크 바로 옆)
        const rearX = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength + trunkLength + rearLength/2;
        ctx.fillStyle = currentCarColor;
        ctx.strokeStyle = '#333333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(rearX - rearLength/2, topBaseY);
        ctx.lineTo(rearX + rearLength/2, topBaseY);
        ctx.lineTo(rearX + rearLength/2, topBaseY - carWidth);
        ctx.lineTo(rearX - rearLength/2, topBaseY - carWidth);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        ctx.font = '16px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('뒷면', rearX, topBaseY + 30);
        
        // === 접착면(Glue Tabs) 추가 - 넓은 접착면과 자르기 선 ===
        const tabWidth = 20; // 접착면 폭을 더 넓게
        
        // 전체 일렬 배치된 부품들의 외곽 경계에 접착면 추가
        const totalWidth = frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength + trunkLength + rearLength;
        const startX = partsStartX;
        const endX = partsStartX + totalWidth;
        
        // === 위쪽 접착면 (전체 위쪽) ===
        ctx.fillStyle = '#FFE6E6'; // 연한 빨간색 배경
        ctx.fillRect(startX, topBaseY - carWidth - tabWidth, totalWidth, tabWidth);
        
        ctx.strokeStyle = '#FF0000'; // 빨간색 테두리
        ctx.lineWidth = 2;
        ctx.setLineDash([5, 5]); // 점선 패턴
        ctx.strokeRect(startX, topBaseY - carWidth - tabWidth, totalWidth, tabWidth);
        
        // "접착면" 텍스트
        ctx.fillStyle = '#FF0000';
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('접착면', startX + totalWidth/2, topBaseY - carWidth - tabWidth/2 + 5);
        
        // === 아래쪽 접착면 (전체 아래쪽) ===
        ctx.fillStyle = '#FFE6E6'; // 연한 빨간색 배경
        ctx.fillRect(startX, topBaseY, totalWidth, tabWidth);
        
        ctx.strokeStyle = '#FF0000'; // 빨간색 테두리
        ctx.lineWidth = 2;
        ctx.setLineDash([5, 5]); // 점선 패턴
        ctx.strokeRect(startX, topBaseY, totalWidth, tabWidth);
        
        // "접착면" 텍스트
        ctx.fillStyle = '#FF0000';
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('접착면', startX + totalWidth/2, topBaseY + tabWidth/2 + 5);
        
        // === 왼쪽 접착면 (앞면 왼쪽) ===
        ctx.fillStyle = '#FFE6E6'; // 연한 빨간색 배경
        ctx.fillRect(startX - tabWidth, topBaseY - carWidth, tabWidth, carWidth);
        
        ctx.strokeStyle = '#FF0000'; // 빨간색 테두리
        ctx.lineWidth = 2;
        ctx.setLineDash([5, 5]); // 점선 패턴
        ctx.strokeRect(startX - tabWidth, topBaseY - carWidth, tabWidth, carWidth);
        
        // "접착면" 텍스트 (세로)
        ctx.save();
        ctx.translate(startX - tabWidth/2, topBaseY - carWidth/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillStyle = '#FF0000';
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('접착면', 0, 0);
        ctx.restore();
        
        // === 오른쪽 접착면 (뒷면 오른쪽) ===
        ctx.fillStyle = '#FFE6E6'; // 연한 빨간색 배경
        ctx.fillRect(endX, topBaseY - carWidth, tabWidth, carWidth);
        
        ctx.strokeStyle = '#FF0000'; // 빨간색 테두리
        ctx.lineWidth = 2;
        ctx.setLineDash([5, 5]); // 점선 패턴
        ctx.strokeRect(endX, topBaseY - carWidth, tabWidth, carWidth);
        
        // "접착면" 텍스트 (세로)
        ctx.save();
        ctx.translate(endX + tabWidth/2, topBaseY - carWidth/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillStyle = '#FF0000';
        ctx.font = 'bold 14px Arial';
        ctx.textAlign = 'center';
        ...
        // === 자르기 선 추가 (각 부품 사이) ===
        ctx.strokeStyle = '#000000'; // 검은색 실선
        ctx.lineWidth = 3;
        ctx.setLineDash([]); // 실선
        
        // 앞면-보닛 사이 자르기 선
        const cut1X = partsStartX + frontLength;
        ctx.beginPath();
        ctx.moveTo(cut1X, topBaseY - carWidth - tabWidth);
        ctx.lineTo(cut1X, topBaseY + tabWidth);
        ctx.stroke();
        
        // 보닛-앞창문 사이 자르기 선
        const cut2X = partsStartX + frontLength + bonnetLength;
        ctx.beginPath();
        ctx.moveTo(cut2X, topBaseY - carWidth - tabWidth);
        ctx.lineTo(cut2X, topBaseY + tabWidth);
        ctx.stroke();
        
        // 앞창문-지붕 사이 자르기 선
        const cut3X = partsStartX + frontLength + bonnetLength + frontWindowLength;
        ctx.beginPath();
        ctx.moveTo(cut3X, topBaseY - carWidth - tabWidth);
        ctx.lineTo(cut3X, topBaseY + tabWidth);
        ctx.stroke();
        
        // 지붕-뒷창문 사이 자르기 선
        const cut4X = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength;
        ctx.beginPath();
        ctx.moveTo(cut4X, topBaseY - carWidth - tabWidth);
        ctx.lineTo(cut4X, topBaseY + tabWidth);
        ctx.stroke();
        
        // 뒷창문-트렁크 사이 자르기 선
        const cut5X = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength;
        ctx.beginPath();
        ctx.moveTo(cut5X, topBaseY - carWidth - tabWidth);
        ctx.lineTo(cut5X, topBaseY + tabWidth);
        ctx.stroke();
        
        // 트렁크-뒷면 사이 자르기 선
        const cut6X = partsStartX + frontLength + bonnetLength + frontWindowLength + roofLength + rearWindowLength + trunkLength;
        ctx.beginPath();
        ctx.moveTo(cut6X, topBaseY - carWidth - tabWidth);
        ctx.lineTo(cut6X, topBaseY + tabWidth);
        ctx.stroke();
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center justify-center px-1 py-1 md:p-2 rounded-xl border-2 transition-all duration-200 h-[85px] md:h-[106px] ${
  ...
  const renderExportStep = () => (
    <div className="space-y-4">
      ...
      <div className="mb-6 mt-10 px-4 md:px-0">
        <PageHeader 
          title="박스카 도안 완성!"
          description="AI가 만든 내 도안을 인쇄하면, 진짜 박스카 만들 준비 끝!"
        />
      </div>
