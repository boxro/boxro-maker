...
export default function StorePageClient() {
  ...
  if (typeof window !== 'undefined') {
    (window as any).__storeIndexCache = (window as any).__storeIndexCache || new Map();
    (window as any).__storeIndexLoaded = (window as any).__storeIndexLoaded || false;
  }
  ...
  const createIndex = async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        (window as any).__storeIndexCache = indexInfo;
  ...
  const shareArticle = async (article: StoryArticle) => {
    try {
      ...
      const shareUrl = `${window.location.origin}/store#card-${article.id}`;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {articles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {/* 링크된 카드가 없을 때만 배너 표시 */}
            {isClient && !window.location.hash.startsWith('#card-') && <BannerDisplay currentPage="store" />}
          ...
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-6 max-w-sm w-full mx-6">
            <div className="text-center">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  onClick={async () => {
                    const shareUrl = `${window.location.origin}/store#card-${selectedArticle.id}`;
                    try {
