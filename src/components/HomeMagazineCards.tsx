...
export default function HomeStoryCards() {
  ...
  const fetchHomeCards = async () => {
    try {
      ...
      // storyArticles와 homeCards 두 컬렉션에서 모두 가져오기
      const [storyQuery, homeCardsQuery] = await Promise.all([
      ...
      // 두 컬렉션 데이터 합치기
      const allArticles = [...storyArticles, ...homeCards];
  ...
  return (
    <>
      {homeCards.map((article) => (
        <Link 
          key={article.id} 
          href={article.url || `/story/${article.id}`} 
